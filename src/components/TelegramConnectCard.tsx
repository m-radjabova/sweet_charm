...
export default function TelegramConnectCard({
  ...
  const connectedAt = formatConnectedAt(info?.telegram_connected_at);
  const tokenExpiresAt = formatTokenExpiry(info?.token_expires_at);
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div className="flex-1">
            ...
            <h3 className="mt-4 text-2xl font-black tracking-tight text-slate-900 sm:text-3xl">
              {isBarber 
                ? "📱 Bot orqali bronlarni kuzating" 
                : "📢 Bot orqali yangiliklardan xabardor bo'ling"}
            </h3>
            
            <p className="mt-2 text-sm leading-relaxed text-slate-600 max-w-xl">
              {isBarber
                ? "Yangi bron qilishlar haqida Telegram orqali tezkor xabar oling. Hech qanday bronni o'tkazib yubormang!"
                : "Sevimli barberlaringizdagi aksiyalar, chegirmalar va yangi xizmatlar haqida birinchilardan bo'lib xabar oling."}
            </p>
          </div>
          ...
          <div className="space-y-5">
            ...
            <div className="rounded-2xl bg-gradient-to-br from-white to-slate-50 p-5 shadow-md ring-1 ring-slate-200/50 transition-all duration-300 hover:shadow-lg">
              ...
              <div className="mt-4 space-y-3">
                {[
                  "QR kodni skaner qiling yoki havolani bosing",
                  "Telegram botda 'Start' tugmasini bosing",
                  "Avtomatik tarzda profilingiz ulanadi",
                ].map((step, idx) => (
            ...
            <div className="rounded-2xl bg-gradient-to-br from-white to-slate-50 p-5 shadow-md ring-1 ring-slate-200/50 transition-all duration-300 hover:shadow-lg">
              ...
              <div className="mt-4 space-y-3">
                ...
                {connected && connectedAt && (
                  <div className="flex items-center justify-between rounded-xl bg-slate-50 p-3">
                    <span className="text-sm font-semibold text-slate-600">Ulangan vaqt:</span>
                ...
                {!connected && tokenExpiresAt && (
                  <div className={`rounded-xl p-3 ${linkExpired ? "bg-rose-50" : "bg-slate-50"}`}>
                    <div className="flex items-center justify-between gap-3">
                      <span className="text-sm font-semibold text-slate-600">Link muddati:</span>
                    ...
                    {linkExpired && (
                      <p className="mt-2 text-xs font-medium text-rose-600">
                        Eski QR link ishlamasligi mumkin. "QR kodni yangilash" tugmasini bosing.
                      </p>
                    )}
                  </div>
                )}
                ...
                {isBarber && (
                  <div className="overflow-hidden rounded-xl bg-gradient-to-r from-sky-50 to-cyan-50 p-3">
                    <div className="flex items-center justify-between">
                      <span className="text-sm font-semibold text-slate-600">📊 Obunachilar soni:</span>
                    ...
                    <p className="mt-1 text-xs text-slate-500">Sizning botingizga ulangan foydalanuvchilar</p>
                  </div>
                )}
              </div>
            </div>
              ...
              >
                {refreshing ? (
                  ...
                  <>
                    ...
                    <span>{connected ? "Yangi ulanish linki" : "QR kodni yangilash"}</span>
                  </>
                )}
              </button>
            </div>
