...
export default function BookingDetails() {
  ...
  const [clientPhone, setClientPhone] = useState(`${UZBEKISTAN_PHONE_PREFIX} `);
  ...
  const phoneDigits = normalizeUzbekPhone(clientPhone);
  const isValid = clientName.trim().length >= 2 && phoneDigits.length === 9;
            ...
            <div className="space-y-5 lg:sticky lg:top-6 lg:h-fit">
              {isLoading ? (
                    ...
                    <div className="relative rounded-[28px] bg-white p-5 shadow-lg transition-all duration-300 group-hover:shadow-xl sm:p-6">
                      <div className="flex items-center justify-between mb-4">
                        ...
                        <div className="flex items-center gap-1 rounded-full bg-emerald-50 px-2 py-1">
                          ...
                          <span className="text-xs font-semibold text-emerald-600">{t("bookingDetails.confirming")}</span>
                        </div>
                      </div>
                ...
                <div className="rounded-2xl bg-red-50 p-6 text-center">
                  ...
                  <p className="mt-2 font-semibold text-red-600">{t("bookingDetails.loadFailed")}</p>
                  <p className="mt-1 text-sm text-red-500">{t("bookingDetails.goBackTryAgain")}</p>
                </div>
              )}
            </div>
                ...
                onSubmit={async (event) => {
                  ...
                  if (isValid) {
                    await createBookingMutation.mutateAsync({
                      ...
                      client_phone: toApiPhone(clientPhone),
                    });
                  }
                }}
              >
                <div className="mb-6">
                  <h3 className="text-xl font-black text-slate-950">{t("bookingDetails.contactInfo")}</h3>
                  <p className="mt-1 text-sm text-slate-500">{t("bookingDetails.contactSubtitle")}</p>
                </div>
                ...
                <div className="mt-5 space-y-2">
                  ...
                  <div className={`group relative transition-all duration-200 ${
                    ...
                    <input
                      value={clientPhone}
                      onChange={(event) => setClientPhone(formatUzbekPhone(event.target.value))}
