    ...
    <div className="barber-theme min-h-screen bg-white px-4 py-20 sm:px-6 md:px-8 lg:px-12">
      <div className="mx-auto max-w-6xl">
        ...
        <section className="rounded-2xl sm:rounded-[30px] border border-white/70 bg-white/88 p-4 shadow-[0_24px_70px_rgba(15,23,42,0.08)] backdrop-blur sm:p-5 md:p-8">
          <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-start sm:gap-6 md:gap-4">
            <div className="flex items-center gap-3 sm:gap-5">
              ...
              <div>
                <p className="text-sm font-medium text-slate-400 sm:text-base md:text-xl">{t("barberDashboard.greeting")}</p>
        ...
        <section className="mt-4 grid grid-cols-3 gap-2 sm:mt-6 sm:gap-3 md:mt-8 md:gap-5">
          <MetricCard icon={<HiMiniUsers className="text-xl sm:text-2xl md:text-3xl" />} value={dashboard?.stats.total ?? 0} label={t("common.today")} tone="primary" />
          <MetricCard icon={<HiMiniCheckBadge className="text-xl sm:text-2xl md:text-3xl" />} value={dashboard?.stats.completed ?? 0} label={t("barberDashboard.done")} tone="success" />
          <MetricCard icon={<HiMiniClock className="text-xl sm:text-2xl md:text-3xl" />} value={dashboard?.stats.pending ?? 0} label={t("barberDashboard.pending")} tone="light" />
        </section>
        ...
        <section className="mt-4 rounded-2xl border border-white/70 bg-white/88 p-4 shadow-[0_20px_60px_rgba(15,23,42,0.06)] sm:mt-6 sm:rounded-[28px] sm:p-5 md:mt-8 md:p-8">
          <div className="flex flex-col justify-between gap-2 sm:flex-row sm:items-center sm:gap-3 md:gap-4">
            <p className="text-base font-black text-slate-700 sm:text-lg md:text-2xl">{t("barberDashboard.progress")}</p>
            <p className="text-sm font-semibold text-slate-400 sm:text-base md:text-xl">
              {t("barberDashboard.completedCount", { completed: dashboard?.stats.completed ?? 0, total: dashboard?.stats.total ?? 0 })}
            </p>
          </div>
        ...
        <section className="mb-4 mt-4 rounded-2xl bg-black p-5 text-white shadow-[0_30px_70px_rgba(15,23,42,0.24)] sm:mb-6 sm:mt-6 sm:rounded-[30px] sm:p-6 md:mt-8 md:p-8">
          <div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between md:gap-6">
            <div className="flex-1">
              <p className="text-xs font-bold uppercase tracking-[0.2em] text-white/45 sm:text-sm md:text-base">{t("barberDashboard.nextClient")}</p>
              {nextBooking ? (
                <>
                ...
                <>
                  <h2 className="mt-4 text-2xl font-black sm:mt-5 sm:text-3xl md:mt-8 md:text-4xl">{t("barberDashboard.noPendingClients")}</h2>
                  <p className="mt-2 text-sm text-white/60 sm:mt-3 sm:text-base md:text-lg">{t("barberDashboard.noPendingClientsText")}</p>
                </>
              )}
            </div>
            ...
            <div className="flex flex-col items-center gap-4 sm:items-end">
              ...
              {nextBooking && (
                ...
                >
                  {completeMutation.isPending ? t("settings.updating") : t("barberDashboard.markComplete")}
                </button>
              )}
            </div>
          </div>
        ...
        >
          <div className="flex items-center gap-3 sm:gap-5">
            ...
            <div>
              <p className="text-lg font-black text-slate-950 sm:text-xl md:text-3xl">{t("barberDashboard.schedule")}</p>
              <p className="mt-0.5 text-xs text-slate-400 sm:mt-1 sm:text-sm md:text-lg">{t("barberDashboard.viewAppointments")}</p>
            </div>
          </div>
        ...
        <section className="mt-6 sm:mt-8 md:mt-10">
          <div className="mb-4 flex flex-col justify-between gap-2 sm:mb-6 sm:flex-row sm:items-center sm:gap-4 md:mb-8">
            <h2 className="text-xl font-black text-slate-950 sm:text-2xl md:text-4xl">{t("barberDashboard.todayAppointments")}</h2>
